...
int Register_Descriptor::return_size(){
...
void Lra_Outcome::optimize_lra(Lra_Scenario lcase, Ast * destination_memory, Ast * source_memory)
{
	...
	case r2m:
		CHECK_INVARIANT(CONTROL_SHOULD_NOT_REACH,
				"r2m scenario does not call for local register allocation");
...
void Machine_Description::initialize_register_table()
{
	...
	spim_register_table[f0] = new Register_Descriptor(f0, "f0", float_num, fixed_reg);
...
void Machine_Description::initialize_instruction_table()
{
	...
	spim_instruction_table[addf] = new Instruction_Descriptor(addf, "add.d", "add.d", "", i_r_o1_op_o2, a_op_o1_o2_r);
	spim_instruction_table[subtract] = new Instruction_Descriptor(subtract, "sub", "sub", "", i_r_o1_op_o2, a_op_o1_o2_r);
	spim_instruction_table[subtractf] = new Instruction_Descriptor(subtractf, "sub.d", "sub.d", "", i_r_o1_op_o2, a_op_o1_o2_r);
	spim_instruction_table[multiply] = new Instruction_Descriptor(multiply, "mul", "mul", "", i_r_o1_op_o2, a_op_o1_o2_r);
	spim_instruction_table[multiplyf] = new Instruction_Descriptor(multiplyf, "mul.d", "mul.d", "", i_r_o1_op_o2, a_op_o1_o2_r);
	spim_instruction_table[divide] = new Instruction_Descriptor(divide, "div", "div", "", i_r_o1_op_o2, a_op_o1_o2_r);
	spim_instruction_table[dividef] = new Instruction_Descriptor(dividef, "div.d", "div.d", "", i_r_o1_op_o2, a_op_o1_o2_r);
	spim_instruction_table[negation] = new Instruction_Descriptor(negation, "uminus", "neg", "", i_r_op_o1, a_op_r_o1);
	spim_instruction_table[negationf] = new Instruction_Descriptor(negationf, "uminus.d", "neg.d", "", i_r_op_o1, a_op_r_o1);
...
Register_Descriptor * Machine_Description::get_new_register(Register_Val_Type  val_type)
{
	Register_Descriptor * reg_desc;
	...
	for (i = spim_register_table.begin(); i != spim_register_table.end(); i++)
	{
		...
		if (reg_desc->is_free() && reg_desc->get_register_type()==val_type)
		{
			reg_desc->set_use_for_expr_result();
			return reg_desc;
		}
